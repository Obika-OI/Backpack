...
import { Award, Download, Settings, FileText, Image as ImageIcon } from 'lucide-react';
...
export const CourseCertificate = ({ course, isStudent, progress }: { course: Course, isStudent: boolean, progress: number }) => {
    ...
    const config = course.certificateConfig || { 
        ...
        orgName: 'Leadpac Digital Learning System',
        gradeLevel: '',
        qualificationTitle: '',
        signatureUrl: '',
        authorizedSealUrl: ''
    ...
    const [qualificationTitle, setQualificationTitle] = useState(config.qualificationTitle || '');
    ...
    const CertificatePreview = () => (
        <div className="max-w-2xl mx-auto aspect-[1.414] bg-white text-slate-900 border-[12px] border-double border-slate-300 p-8 flex flex-col relative mb-8 shadow-xl">
            ...
            <div className="text-center flex-1 flex flex-col justify-center">
                <div className="text-sm uppercase tracking-widest text-slate-500 font-bold mb-6">{orgName || 'Organization Name'}</div>
                ...
                <div className="text-xl font-bold text-slate-800 mb-2">{qualificationTitle || course.title}</div>
    ...
    if (isStudent) {
        if (!config.enabled) {
            return <div className="p-8 text-center text-slate-400">Certificates are not enabled for this course.</div>;
        }
        if (progress < 100) {
            return (
                <div className="p-8 text-center">
                    ...
                    <p className="text-slate-400">Complete all modules and assessments (100%) to unlock your certificate.</p>
        ...
        return (
            <div className="p-8 text-center">
                ...
                <p className="text-slate-300 mb-6">You have successfully completed this course and earned your certificate.</p>
                <CertificatePreview />
                <button onClick={handleDownload} className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl flex items-center justify-center mx-auto transition-colors">
    ...
    return (
        <div className="p-6 md:p-8 space-y-6">
            <div className="flex justify-between items-center mb-6 border-b border-slate-700 pb-4">
                <h2 className="text-xl font-bold text-white flex items-center"><Award className="w-5 h-5 mr-2 text-indigo-400" /> Certificate Settings</h2>
                {!isEditing && (
                    <button onClick={() => setIsEditing(true)} className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-sm font-medium transition flex items-center">
            ...
            {isEditing ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="space-y-4">
                        <label className="flex items-center space-x-3 bg-slate-800 p-4 rounded-xl border border-slate-700 cursor-pointer">
                            <input type="checkbox" checked={enabled} onChange={e => setEnabled(e.target.checked)} className="w-5 h-5 rounded border-slate-600 text-indigo-600 focus:ring-indigo-500 bg-slate-900" />
                            <span className="text-white font-medium">Enable Certificates for this Course</span>
                        </label>
                        ...
                        {enabled && (
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input type="text" value={orgName} onChange={e => setOrgName(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-indigo-500" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-300 mb-1">Certificate Header Text</label>
                                    <input type="text" value={customText} onChange={e => setCustomText(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-indigo-500" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-300 mb-1">Qualification / Course Title</label>
                                    <input type="text" value={qualificationTitle} onChange={e => setQualificationTitle(e.target.value)} placeholder="Leave blank to use course name" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-indigo-500" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-300 mb-1">Grade Level (Optional)</label>
                                    <input type="text" value={gradeLevel} onChange={e => setGradeLevel(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-indigo-500" />
                                </div>
                                
                                <div className="grid grid-cols-2 gap-4 pt-2">
                                    <div>
                                        <label className="block text-sm font-medium text-slate-300 mb-2">Authorized Signature</label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-300 mb-2">Official Seal</label>
                        ...
                        <div className="flex space-x-3 pt-6">
                            <button onClick={handleSave} className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-bold transition">Save Design</button>
                            <button onClick={() => setIsEditing(false)} className="px-6 py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-lg font-bold transition">Cancel</button>
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    {config.enabled && (
                        <div className="mt-8">
                            <h3 className="text-lg font-bold text-white mb-6">Certificate Preview</h3>
                            <CertificatePreview />
                        </div>
                    )}
                </div>
