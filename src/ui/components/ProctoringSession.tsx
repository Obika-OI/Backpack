...
export const ProctoringSession = ({ assessmentTitle, onComplete }: { assessmentTitle: string, onComplete: () => void }) => {
    ...
    if (!isProctoring) {
        return (
            <div className="bg-slate-50 dark:bg-slate-900 border border-amber-500/30 rounded-xl p-6 text-center">
                <ShieldAlert className="w-12 h-12 text-amber-500 mx-auto mb-4" />
                <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Proctored Assessment: {assessmentTitle}</h3>
                ...
                <button 
                    ...
                    className="px-6 py-3 bg-amber-600 hover:bg-amber-500 text-slate-900 dark:text-white rounded-lg font-bold transition-colors shadow-lg shadow-amber-500/20 flex items-center justify-center mx-auto"
                >
        ...
        <div className="bg-slate-950 border border-emerald-500/30 rounded-xl p-6">
            <div className="flex items-center justify-between mb-6">
                ...
                <button 
                    ...
                    className="px-4 py-2 bg-red-600 hover:bg-red-500 text-slate-900 dark:text-white rounded-lg text-sm font-bold transition-colors"
                >
                    End Session
                </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-slate-50 dark:bg-slate-900 rounded-lg overflow-hidden border border-slate-800 relative aspect-video">
                    <video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover"></video>
                    <div className="absolute bottom-2 left-2 bg-black/60 px-2 py-1 rounded text-xs text-slate-900 dark:text-white flex items-center">
