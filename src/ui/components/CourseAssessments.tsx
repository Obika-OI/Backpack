...
export const CourseAssessments = ({ courseId, isStudent }: { courseId: string, isStudent: boolean }) => {
        ...
        return (
            <div className="space-y-6">
                <div className="bg-indigo-900/40 p-6 rounded-2xl border border-indigo-500/30 flex items-center justify-between">
                    <div>
                        <h3 className="text-xl font-bold text-white mb-1 flex items-center"><Award className="w-5 h-5 mr-2 text-amber-400" /> Academic Transcript</h3>
                        <p className="text-sm text-indigo-200">Track your performance across assignments and exams.</p>
                    </div>
                    <div className="text-right">
                        <div className="text-3xl font-black text-white">{totalScore} <span className="text-lg text-indigo-300 font-medium">/ {totalMaxScore}</span></div>
                ...
                <div className="space-y-4">
                    {courseAssessments.length === 0 ? (
                        <div className="text-center py-10 text-slate-400 bg-slate-900/50 rounded-xl">No assessments scheduled yet.</div>
                    ) : (
                        courseAssessments.map(ass => {
                            const sub = mySubmissions.find(s => s.assessmentId === ass.id);
                            return (
                                <div key={ass.id} className="bg-slate-900 border border-slate-700 rounded-xl p-6">
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            ...
                                            <div className="flex space-x-3 mt-1 text-sm text-slate-400">
                                        ...
                                        <div className="mt-4 pt-4 border-t border-slate-800">
                                            {(ass.type === 'exam' || ass.type === 'test') && activeProctoringId !== ass.id ? (
                                                <button 
                                                    ...
                                                    className="w-full px-4 py-3 bg-amber-600 hover:bg-amber-500 text-white rounded-lg font-bold transition flex items-center justify-center"
                                                >
    ...
    return (
        <div className="space-y-8">
            <form onSubmit={handleCreateAssessment} className="bg-slate-900 border border-slate-700 p-6 rounded-2xl">
                <h3 className="text-lg font-bold text-white mb-4 flex items-center"><Plus className="w-5 h-5 mr-2 text-indigo-400" /> Create New Assessment</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label className="block text-xs font-semibold text-slate-400 mb-1">Title</label>
                        <input required type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white focus:ring-1 focus:ring-indigo-500 outline-none" placeholder="e.g. Midterm Exam" />
                    </div>
                    <div>
                        <label className="block text-xs font-semibold text-slate-400 mb-1">Type</label>
                        <select value={type} onChange={e => setType(e.target.value as 'assignment' | 'test' | 'exam' | 'project')} className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white focus:ring-1 focus:ring-indigo-500 outline-none">
                    ...
                    <div>
                        ...
                        <input required type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white focus:ring-1 focus:ring-indigo-500 outline-none" />
                    </div>
                    <div>
                        <label className="block text-xs font-semibold text-slate-400 mb-1">Max Score</label>
                        <input required type="number" min={1} value={maxScore} onChange={e => setMaxScore(Number(e.target.value))} className="w-full bg-slate-800 border border-slate-700 rounded-lg p-2.5 text-white focus:ring-1 focus:ring-indigo-500 outline-none" />
                    </div>
                </div>
                {(type === 'assignment' || type === 'project') && (
                    <div className="flex items-center space-x-2">
                        <input type="checkbox" id="isGroup" checked={isGroup} onChange={e => setIsGroup(e.target.checked)} className="w-4 h-4 text-indigo-600 bg-slate-800 border-slate-700 rounded focus:ring-indigo-500 focus:ring-offset-slate-900" />
                        <label htmlFor="isGroup" className="text-sm text-slate-300 font-medium">Group Assessment (Students can work in teams)</label>
                    </div>
                )}
                <button type="submit" className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-bold transition w-full sm:w-auto">
                    Publish Assessment
                </button>
            </form>

            <div className="space-y-6">
                <h3 className="text-xl font-bold text-white">Manage Assessments</h3>
                {courseAssessments.length === 0 ? (
                    <div className="text-center py-10 bg-slate-900/50 rounded-xl border border-slate-800 text-slate-400">No assessments created yet.</div>
                ) : (
                    courseAssessments.map(ass => {
                            ...
                            <div key={ass.id} className="bg-slate-900 border border-slate-700 rounded-xl p-6">
                                <div className="flex justify-between items-center mb-4">
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-400 capitalize">{ass.type} • Max: {ass.maxScore} pts • Due: {new Date(ass.dueDate).toLocaleDateString()}</p>
                                    </div>
                                    <button 
                                        ...
                                        {subs.length === 0 ? (
                                            ...
                                            <div className="space-y-3">
                                                {subs.map(sub => {
                                                    ...
                                                    return (
                                                        <div key={sub.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-slate-800/50 rounded-lg">
                                                                ...
                                                                {sub.status === 'graded' ? (
                                                                    ...
                                                                    <>
                                                                        <input 
                                                                            ...
                                                                            placeholder="Score"
                                                                            className="w-20 bg-slate-900 border border-slate-700 rounded px-2 py-1.5 text-white outline-none focus:border-indigo-500 text-center"
                                                                        />
                                                                        <button 
                                                                            onClick={() => handleGradeSubmission(sub.id)}
                                                                            className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded text-sm font-bold transition"
                                                                        >
                                                                            Grade
                                                                        </button>
                                                                    </>
